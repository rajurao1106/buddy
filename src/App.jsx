...
export default function App() {
  ...
  const socket = io("http://rajurao1106:3000"); // Use your backend URL

  useEffect(() => {
    // Fetch existing tasks
    const fetchTasks = () => {
      axios
        .get("http://rajurao1106:3000/taskmanager/alltasks")
        .then((response) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axios.post("http://rajurao1106:3000/taskmanager/newtask", task); // Add task
      setTaskDescription("");
      setTaskTitle("");
    } catch (error) {
